...
Class Action {
	...
	function login(){
		...
		if($qry->num_rows > 0){
			...
			foreach ($result as $key => $value) {
		      ...
		      //  $qry1 = $this->db->query("SELECT user_id FROM `user_login` WHERE user_id = '".(int)$result['id']."' ");	
			  //   $qry1 = $this->db->query("SELECT user_id FROM `user_login` WHERE user_id = '" . (int)$result['id'] . "' AND DATE(login_date) = '$currentDate'");

		    	//$qry2 = $this->db->query("SELECT login_date FROM `user_login` WHERE user_id = '".(int)$result['id']."' ");
	...
	function save_menu(){
		...
		echo '<script>';
        echo 'console.log("This message is logged to the JavaScript console.");';
        echo '</script>';  
		$data = " name = '$name' ";
		//$data .= ", price = '$price' ";
...
$result = $this->db->query("SELECT COUNT(*) AS new_order_count FROM orders");
